...
# Smoothing kernel from phantom from the phantom paper (Sect. 2.1.5, 2.1.6)
...
def getRhoSmoothingKernel(closest_points, W_list,param, massPerRho):
...
#           shape == 'z' --> planar slice: orbital plane
...
def getSmoothingKernelledPix(n, neighbours, data, params, r, shape, bound):
    ...
    for i in range(len(params)):
        ...
        results[str(params[i])] = getRhoSmoothingKernel(closest_points, W_ab, data[params[i]], mPrho_closest_points) 
    ...
    if shape == 'z' or shape == 'y' or shape == 'line_x' or shape == 'line_y' or shape == 'line_z':
        ...
        #print(results, pixCoord[0], pixCoord[1], pixCoord[2])
