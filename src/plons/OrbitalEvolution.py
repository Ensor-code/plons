...
def plot_orbit(data, setup,loc):
    ...
    if setup['triple_star']==True:
        ...
        temp=-1#int(np.round(len(xCOM_out)/6))
        #Mass centers are not needed
        #ax.plot(xCOM_in[:temp]/cgs.au,yCOM_in[:temp]/cgs.au, color = 'navy', label = 'CoM_inner')
        #ax.plot(xCOM_out[:temp]/cgs.au,yCOM_out[:temp]/cgs.au,'*', color = 'navy', label = 'CoM_outer', markersize = 5)
        #1+2 triples:
        #ax.plot(xc_in[1:temp]/cgs.au,yc_in[1:temp]/cgs.au,linestyle='dotted', c = 'b', label ='Outer 0.6'+r'$M_{\odot}$'+' companion')
        #ax.plot(xc[1:temp]/cgs.au,yc[1:temp]/cgs.au, linestyle='--',c = 'orange', label = 'Outer 0.6'+r'$M_{\odot}$'+' companion')
        #2+1 triples:
        ax.plot(xc_in[1:temp]/cgs.au,yc_in[1:temp]/cgs.au,linestyle='dashed', c = 'b', label ='Inner 0.6'+r'$M_{\odot}$'+' companion')
        ax.plot(xc[1:temp]/cgs.au,yc[1:temp]/cgs.au, linestyle=':',c = 'orange', label = 'Outer 1.2'+r'$M_{\odot}$'+' companion')
    elif setup['quadruple_star']==True:
        print('quad star setup yes!')
        xc_in = data['posComp_in'].transpose()[0]
        yc_in = data['posComp_in'].transpose()[1]
        xc_in2 = data['posComp_in2'].transpose()[0]
        yc_in2 = data['posComp_in2'].transpose()[1]
        temp=-1#int(np.round(len(xCOM_out)/6))
        ax.plot(xc_in[1:temp]/cgs.au,yc_in[1:temp]/cgs.au,linestyle='dotted', c = 'k', label ='Inner 0.6'+r'$M_{\odot}$'+' companion')
        ax.plot(xc[1:temp]/cgs.au,yc[1:temp]/cgs.au, linestyle='--',c = 'b', label = 'Outer 0.6'+r'$M_{\odot}$'+' companion')
        ax.plot(xc_in2[1:temp]/cgs.au,yc_in2[1:temp]/cgs.au, linestyle='--',c = 'g', label = 'Outer 0.6'+r'$M_{\odot}$'+' companion')
    else:
        ...
        ax.plot(xc[1:]/cgs.au,yc[1:]/cgs.au, c = 'black', label = 'companion')

    ax.plot(xa[1:]/cgs.au,ya[1:]/cgs.au, c = 'k', label ='Inner 2.1'+r'$M_{\odot}$'+' AGB star'   )
...
def plotMassAccr(setup, sinkData, run, loc):
    ...
    plt.plot(sinkData['time'],  sinkData['maccrComp']/cgs.Msun, color = 'black', linestyle = 'solid',label='companion 1')
    if setup['triple_star']==True:
        plt.plot(sinkData['time'],  sinkData['maccrComp_in']/cgs.Msun, color = 'blue', linestyle = 'dashed', label = 'companion 2')
        #comp_out       = mlines.Line2D([],[], color = 'black', linestyle = 'solid', linewidth = 0.5, label = 'companion 2')
        #comp_in        = mlines.Line2D([],[], color = 'blue', linestyle = 'solid', linewidth = 0.5, label = 'companion 1')
    ...
    plt.yscale('log')
    plt.title('Total accreted mass by the companion(s)', fontsize = 18)
    #if setup['triple_star']==True:
...
def plotMassAccrRate(setup, sinkData, run, loc):
    ...
    plt.plot(t_yrs[:-1], accrRates,color = 'black', linestyle = 'solid',label='companion 1')
    if setup['triple_star']==True:
        plt.plot(t_yrs[:-1], accrRates_in,color = 'blue', linestyle = 'dashed',label='companion 2')
        comp_out       = mlines.Line2D([],[], color = 'black', linestyle = 'solid', linewidth = 0.5, label = 'outer comp')
        comp_in        = mlines.Line2D([],[], color = 'blue', linestyle = 'solid', linewidth = 0.5, label = 'inner comp')
        handles_ap    = [periodLine,comp_in,comp_out]
    #else:
        #handles_ap    = [apaLine, perLine]
...
def plotOrbVel(setup,sinkData, run, loc):
    ...
    ax.plot(t, sinkData['v_orbComp_t'][1:]/cgs.kms, label ='companion', c='black')

    if setup['triple_star']==True:
        ax.plot(t, sinkData['v_orbComp_in_t'][1:]/cgs.kms, label ='inner companion', c='blue')
...
def plotOrbRad(setup,sinkData, run, loc):
    ...
    if setup['triple_star']==True:
        ...
        ax.plot(t, rc_out, label= 'r outer comp',c='black',ls='dashed')
        ax.plot(t, rc_in, label= 'r inner comp',c='blue',ls='dotted')
        ...
        ax.plot(t, sinkData['rComp'][1:]/cgs.au, label= 'r comp', c='black')
...
def plotOrbRadSeperate(setup,sinkData, run, loc):
    ...
    if setup['triple_star']==True:
        ...
        rc_in  = sinkData['rComp_in'][1:] /cgs.au
        rc_out = sinkData['rComp'][1:] /cgs.au
        ax2.plot(t, rc_out, label= 'r outer comp',c='black')
        ax2.set_ylabel('r out comp [au]', fontsize = 12)
        ax3.plot(t, rc_in, label= 'r inner comp',c='blue')
