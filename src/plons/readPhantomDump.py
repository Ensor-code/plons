...
# Create an empty dictionary to store the time taken for each function
time_dict = {}
...
def read_fortran_record(f, type, memorymap=True):
  ...
  time_taken = time.time() - start_time
  time_dict['read_fortran_record'] = time_taken
  return data
# Read string
def read_string(f):
    start_time = time.time()
    line = read_fortran_record(f, 'S1')
    time_taken = time.time() - start_time
    time_dict['read_string'] = time_taken
    return line.tobytes().decode('utf-8')
...
def read_names(f):
  ...
  if n>0:
    ...
    names = []
  time_taken = time.time() - start_time
  time_dict['read_names'] = time_taken
  return names
...
def read_fortran_record_with_names(f, type):
  ...
  if len(names)>0:
    ...
    while i<len(names):
      ...
      i += c
  time_taken = time.time() - start_time
  time_dict['read_fortran_record_with_names'] = time_taken
  return out
def read_dump(filename, memorymap=True):
  """ Read a phantom dump
  ...
  time_dict['Opening_file'] = time_taken2
  ...
  time_dict['Repeated reads'] = time_taken2
  # Number of blocks
  ...
  for iblock in range(nblocks):
      ...
      block['nsingle'] = n[8]
      blocks.append(block)
  time_takena = time.time() - start_timea
  time_dict['Dump_blocksa'] = time_takena
  start_time=time.time()
  # Read blocks
  for b in blocks:
    b['data'] = {}
    start_timeb = time.time()
    for _ in range(b['nintdb'] + b['ndouble'] + b['nsingle']):
        column_name = read_string(f).strip()
        column_type = 'i8' if _ < b['nintdb'] else ('f8' if _ < b['nintdb'] + b['ndouble'] else 'f4')
        column_data = read_fortran_record(f, column_type, memorymap)
