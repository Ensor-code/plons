...
def getEverything(mass, theta, rho): 
    ...
    indPi2Ordered    = to.find_nearest(thetaOrdered, np.pi/2)
    ...
    indPi2Reversed   = to.find_nearest(thetaReversed,np.pi/2)
    ...
    ThetaBins = to.create_bins(lower_bound=int(100 * np.pi/2), width=1, quantity=number_of_bins )
    ...
    for index in range(len(thetaPi2_Pi)):
        ...
        bin_index = to.find_bin(100*thetaPi2_Pi[index],ThetaBins)
    ...
    for index in range(len(thetaPi2_0)):
        ...
        thetaBin = np.pi/2 + x
        bin_index = to.find_bin(thetaBin*100, ThetaBins)
...
#     rhoMean = [0] * len(rhoBinned)
    ...
    rhoMeanSmoothened = to.smoothen(rhoMean,4)
