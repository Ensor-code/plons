...
def main_terminalVelocity(setup, dump, sinkData, outputloc, run):
    ...
    with open (title,'w') as f:
        ...
        f.write('Orbital velocity:          '+str(round(dump['v_orbAGB'    ]*1e-5, 2))+' km/s   \n' )
        f.write('\n')
        if single_star == False:    
            f.write('Companion mass:            '+str(round(setup['massComp_ini']     , 2))+' Msun\n')
            f.write('Orbital separation:        '+str(round(setup['sma_ini'     ]     , 2))+' au  \n') 
            f.write('Orbital velocity:          '+str(round(dump['v_orbComp'    ]*1e-5, 2))+' km/s\n')
            f.write('Capture radius:            '+str(round(setup['Rcap'        ]     , 2))+' au  \n')
            f.write('Hill radius:               '+str(round(dump['rHill'        ]/cgs.AU_cm(), 2))+' au  \n')
        if single_star == True: 
